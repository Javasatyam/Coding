package string;

import java.io.FileInputStream;
import java.io.FileNotFoundException;

public class Demo {
	String greeting = "Hello world!"; // literal 1
	String s=new String("Satyam");// 2
	  
	int x=5;
	int y=5;
//	String xyz=new String("satyam");
//    String xyz1=new String("satyam");
	String zz="satyam";
	String xyz1="satyam";

  
	void show()
	{
//		char c='a';  
//		char b='a';
//		Character charobj=c; 
//		Character s=b;
	
		if(zz==xyz1)
		{
			System.out.println("matched");
		}
		else
		{
			System.out.println("NO matched");
		}
	}
	
	
	
	
	 
//	rahul ds=new rahul();
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		
//		String s12="java string";    
//		// The string s1 does not get changed, even though it is invoking the method      
//		// concat(), as it is immutable. Therefore, the explicit assignment is required here.  
//		s12.concat("is immutable");    
////		System.out.println(s12);    
//		s12=s12.concat(" is immutable so assign it explicitly");    
//		System.out.println(s12);   
//		s.show();
		//
//		StringBuffer s2=new StringBuffer("Satyam");
//		System.out.println(s2);
//	    s2.append("singh");
//	    System.out.println(s2);
	   
	    
	    	 int a=10;
	 	    int b=0;
	 	    int c=a/b;
	    
	   
//	    try {
//			FileInputStream fis=new FileInputStream("d:/abc.text");
//		} catch (FileNotFoundException e) {
//			// TODO Auto-generated catch block
//			e.printStackTrace();
//		}
		System.out.println("Satyam");
	

	}

}
